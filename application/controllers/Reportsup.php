...
class Reportsup extends CI_Controller{
    ...
    public function reportedis(){
        ...
        $data['res'] = $this->reportsup_model->tabladis($carnet, -1);
    ...
    public function reportedep(){
        ...
        $data['res'] = $this->reportsup_model->tabladep($carnet, -1);
        $this->load->view('template/template');
...
//////////////////***************ADMIN BEGIN**********************////////////////////////////
    public function admin(){
        ...
        $this->load->view('template/template');
        $this->load->view('sis_vw/segadmin', $data);
        $this->load->view('template/footer');
    }
    public function llenaMaestros(){
        $this->valido();
        $uesel = $this->input->post('uesel');
        $maestros = $this->reportsup_model->tablaue('', $uesel);
        if($maestros){
    ...
    public function llenaDepto(){
        ...
        $maestros = $this->reportsup_model->tabladep('', $deptosel);
        if($maestros){
            echo json_encode($maestros);
        }
    }
    public function llenaDist(){
        $this->valido();
        $distsel = $this->input->post('distsel');
        $maestros = $this->reportsup_model->tabladis('', $distsel);
        if($maestros){
            echo json_encode($maestros);
        }
    }
////////////////********************END****************///////////////////
    ...
    public function arma_tabladis(){
        ...
        $res = $this->reportsup_model->tabladis($this->session->userdata('username'), $datosDis->cod_dis);
        foreach ($res as $value) {
            $cad.="<tr>";
    ...
    public function arma_tablaue(){
        ...
        $res = $this->reportsup_model->tablaue($this->session->userdata('username'), $datosUE->cod_ue);
        foreach ($res as $row) {
            $cad.='<tr><td>'.$i.'</td>';
